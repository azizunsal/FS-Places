...
class AutoCompleteInput extends React.Component {
    ...
    console.log('this.props = ', this.props);
		...
		this.client = new MapboxClient(Config.Mapbox.accessToken);
  ...
  handleItemClick(item) {
    ...
    // const path = `/ll/${item.place_name}`;
    const ll = lat + ',' + lng;
    console.warn("ll ", ll);
    const encodedLL = encodeURIComponent(ll);
    console.log("ll is ", encodedLL);
    console.log("encodedLL is ", encodedLL);
    const path = `/ll/${ll}/venues`;
    ...
    let results;
		...
		return(
			<div className="row">
				...
				/>
        <span className='locationReset'> </span>
				{results}
			</div>
		);
	}
